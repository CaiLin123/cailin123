...
import zuoye.test.instance.Person;
import zuoye.test.data.Data;
...
public class Input {
    public Person addNickName(Person person, String nickName) throws IOException, DocumentException {
        boolean is ;
    if(!(is = Pattern.matches("[A-Za-z].*", nickName))){
    ...
    public Person register(String name, String possWord) throws DocumentException, IOException {
        ...
        Boolean isM;
        if (!((isM = Pattern.matches("[1]\\d{10}", name)) || (isM = Pattern.matches("^[a-zA-Z0-9_-]+@[a-zA-Z0-9_-]+(\\.[a-zA-Z0-9_-]+)+$", name)))) {
            System.out.println("请输入手机号或邮箱号");
            return null;
        }

        if (!((isM = Pattern.matches(".*[a-z]+.*", possWord))&&
                (isM = Pattern.matches(".*[A-Z]{1,}.*", possWord))&&
                        (isM = Pattern.matches(".*[0-9]{1,}.*", possWord)))){
    ...
    public Person login(String name, String possWord) throws DocumentException {
        Map<String, Person> map = Data.get();
        if (map != null) {
            Set<String> keySet = map.keySet();
            for (String s : keySet) {
